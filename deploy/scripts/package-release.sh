...
#
# The CLI ships from opencrew-cli and is packaged by that repo, because neither
...
#
#   usage: package-release.sh <os> <arch> <opencrew-server> <web-dir>
set -eu

if [ "$#" -ne 4 ]; then
  echo "usage: package-release.sh <os> <arch> <opencrew-server> <web-dir>" >&2
  exit 2
fi
OS=$1
ARCH=$2
...
NAME="opencrew-server_${OS}_${ARCH}"

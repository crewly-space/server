...
import { CrewlyClient } from './client.js';
...
describe('CrewlyClient', () => {
  it('exposes every resource wired to the same base URL', async () => {
    ...
    const client = new CrewlyClient({ baseUrl: 'http://localhost:4000', fetchImpl });
  ...
  it('ws() returns a fresh WsClient sharing the same base URL', () => {
    ...
    const client = new CrewlyClient({ baseUrl: 'http://localhost:4000' });
